<template>
  <div class="reply-comment-wrap">
    ...
    <el-row class="mbt20">
      <el-col :xs="20" :sm="16" :md="12" :lg="9" :xl="6">
        <el-input placeholder="请输入内容" v-model="keywords" class="input-with-select" @keyup.enter.native="searchReplyCom">
          <el-select v-if="$route.name=='staUserPepper'" v-model="selectType" slot="prepend" placeholder="请选择">
    ...
    <el-pagination
      ...
      :current-page="pepperCommentList.pageNum"
      :page-size="pepperCommentList.pageSize"
      layout="total, prev, pager, next, jumper"
      :total="pepperCommentList.total">
    </el-pagination>
  ...
  export default{
    ...
    methods:{
      getPepperStatisticList(){
        ...
        this.$ajax("/admin/getAdminGoldenticketlogList",this.searchForm,res=>{
          if(res.returnCode===200){
            this.pepperCommentList = res.data
          }else if(!res.data){
            this.pepperCommentList = {}
          }
        })
      },
      searchReplyCom(){
        if(this.$route.params.page==1){
          this.getPepperStatisticList()
        }else {
    ...
    created(){
      if(this.$route.name==='staUserPepper'){
