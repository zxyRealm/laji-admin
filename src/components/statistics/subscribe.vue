<template>
  <div class="reply-comment-wrap">
    ...
    <el-row class="mbt20">
      <el-col :xs="20" :sm="16" :md="12" :lg="9" :xl="6">
        <el-input placeholder="请输入内容" v-model="keywords" class="input-with-select" @keyup.enter.native="searchReplyCom">
          <el-select v-if="$route.name=='staUserSubscribe'" v-model="selectType" slot="prepend" placeholder="请选择">
    ...
    <el-table
      ref="multipleTable"
      :data="subscribeCommentList.list"
      border
      style="width: 100%">
      <el-table-column
        type="selection">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
           {{ scope.row.bookChapterName }}(id:{{ scope.row.bookChapterId }})
        </template>
      </el-table-column>
      ...
      <el-table-column
        label="时间">
        <template slot-scope="scope">
          <span>{{scope.row.subscriptionDateTime|time('long')}}</span>
        </template>
      </el-table-column>
    ...
    <el-pagination
      ...
      :current-page="subscribeCommentList.pageNum"
      :page-size="subscribeCommentList.pageSize"
      layout="total, prev, pager, next, jumper"
      :total="subscribeCommentList.total">
    </el-pagination>
  ...
  export default{
    ...
    methods:{
      getSubscribeStatisticList(){
        ...
        this.$ajax("/admin/getAdminSubscriptionList",this.searchForm,res=>{
          if(res.returnCode===200){
            this.subscribeCommentList = res.data
          }else if(!res.data){
            this.subscribeCommentList = {}
          }
        })
      },
      searchReplyCom(){
        if(this.$route.params.page==1){
          this.getSubscribeStatisticList()
        }else {
    ...
    created(){
      if(this.$route.name==='staUserSubscribe'){
